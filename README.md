...
- **모던 글래스** — Aero보다 매트한 2020년대식 프로스티드 글래스, Pretendard 폰트
  (Plex Sans 대신 — "개발자 문서" 느낌을 빼기 위함). 사이트 액센트는 웜그레이
  뉴트럴로, 브랜드 블루는 캐릭터 전용(`--color-character`, `css/tokens.css`의
  `--color-accent`와 분리)으로 남겨서 "다 파랗다"는 인상 없이 캐릭터만 유일한
  포인트 컬러가 되도록 했습니다. 헤딩 앞 사각 마커도 제거. Pretendard는
