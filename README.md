...
- **모던 글래스** — Aero보다 매트한 2020년대식 프로스티드 글래스, Pretendard 폰트
  (Plex Sans 대신 — "개발자 문서" 느낌을 빼기 위함). Pretendard는 jsDelivr CDN에서
  로드하는데, 이 저장소를 다루는 클로드 세션의 네트워크 정책상 jsDelivr가 막혀
  있어 세션 내에서는 실제 폰트 렌더링을 스크린샷으로 확인하지 못했습니다 —
  실제 방문자에게는 문제없는 표준 공식 임베드이니, 배포 후 직접 확인해 주세요.
