...
assets/images/Aero.png  히어로 배경 이미지 (첫 화면의 하늘·들판)
...
- **풀스크린 씬** — 히어로가 `100dvh`(화면 높이 전체)를 채우고, 네비/마퀴/
  캐릭터/소개 카드가 전부 그 위에 얹힙니다. 씬 아트는
  `assets/images/Aero.png`(1280×640 픽셀아트, 지평선은 위에서 약 80% 지점).
  배치는 `.hero__scene-art`:
  - 이미지를 **아래에 붙여** 두므로 화면비가 어떻든 잔디가 항상 바닥에 남습니다.
  - 이미지 위쪽 공간은 `--scene-sky`가 이어받습니다. 이미지 맨 위 30줄이 순수
    하늘색이라 같은 값을 쓰면 이음새가 보이지 않습니다(이미지 로딩 전 배경도 겸함).
  - 높이는 `--art-h`. 이미지가 2:1이라 가로를 꽉 채우면 높이가 `50vw`인데,
    좁은 화면에서 그대로 두면 잔디가 너무 얇아 캐릭터가 설 자리가 없어서 최소
    `24rem`을 보장하고(그만큼 좌우가 잘림), 세로로 긴 화면에서는 하늘만 휑하게
    남지 않도록 `55dvh`까지 키웁니다.
  - 캐릭터와 소개 카드의 바닥선도 이 `--art-h` 기준입니다(씬 높이 % 가 아니라).
    잔디 밴드가 아트 높이의 아래 20%라, 발을 그 한가운데(`--art-h * 0.1`)에 둡니다.

  **이미지를 바꾸려면** 같은 경로에 넣고, 지평선 비율이 다르면 위 배수들과
  `css/tokens.css`의 `--scene-sky`(새 이미지의 하늘색)를 다시 맞춰 주세요.
