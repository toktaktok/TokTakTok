...
- **풀스크린 씬** — 히어로가 `100dvh`(화면 높이 전체)를 채우고, 네비/소개 블록/
  캐릭터가 전부 그 위에 얹힙니다. 씬 아트는 **아래쪽 언덕 띠 하나뿐**
  ...
  - 캐릭터의 바닥선도 이 `--art-h` 기준입니다(씬 높이 % 가 아니라). 평평한 앞쪽
    지면이 띠 높이의 아래 52%라, 그 안쪽(`--art-h * 0.18`)에 발을 둡니다.

  **아트를 바꾸려면** `index.html`의 `.hero__scene-art` SVG를 갈아 끼우고,
  능선 비율이 달라지면 위 배수들과 `css/tokens.css`의 `--scene-hill-*`를 맞춰 주세요.
- **소개 블록** (`.hero__copy`) — 이름·직함 / 헤드라인 / 소개 / 메타가 하나의
  세로 흐름으로, 씬의 **위–가운데 왼쪽**에 앉습니다. 위에서부터 자리를 잡고
  캐릭터는 아래에서 잡으므로 좁은 화면에서도 서로 부딪히지 않습니다.
  - 헤드라인은 `data/profile.js`의 `headline`을 그대로 쓰는 **진짜 `<h1>`**이고,
    폰트는 **Black Han Sans**(Google Fonts, `index.html`에서만 로드)입니다.
    전에는 이 글자가 화면 가로를 가득 채우며 흐르는 롤링 마퀴(장식,
    `aria-hidden`)였고 실제 `<h1>`은 `sr-only`였는데, 그러면 화면에서 가장 큰
    글자가 양끝이 잘려 문장이 완성되지 않는 장식이라 위계가 뒤집혀 있었습니다.
    롤링을 걷어내고 글자만 헤드라인으로 승격해서, 서체의 개성은 그대로 두고
    가장 큰 글자가 실제로 읽히게 했습니다(자동으로 도는 콘텐츠가 없어져
    WCAG 2.2.2 정지 수단 요건도 함께 사라짐).
  - 유리 패널은 쓰지 않습니다. 배경이 밝은 종이라 유리는 경계가 거의 안 보여
    무게가 없었습니다 — 글자가 종이 위에 바로 앉습니다.
- **캐릭터 도킹** — 첫 진입 시 마스코트가 들판 위(씬 좌표 `.hero__dock`)에
  ...
  **캐릭터가 실제로 그 자리까지 걸어갑니다** — 걷는 모션(`pix--travel`)을
  ...
  - **비행은 언제나 `body`의 fixed 컨텍스트에서** 하고, 씬 안(`.hero__dock`)
    ...
    대개 씬 아래쪽), `.hero__dock`이 소개 블록보다 아래면 그 뒤로 지나갑니다.
