...
data/career.js      경력 행 (본문 섹션 + 히어로 "경력 한눈에 보기" 팝업 공용)
...
- **들판 배경** — XP Bliss를 오마주한 하늘+언덕. 외부 이미지가 아니라
  ...
  `css/tokens.css`의 `--scene-*` 토큰으로 조절합니다.
- **캐릭터 도킹** — 첫 진입 시 마스코트가 들판 위(씬 좌표 `.hero__dock`)에
  서서 웃으며 손을 흔들고, 아래로 스크롤하면 평소의 우하단 도우미 위치로
  돌아갑니다(다시 올라오면 재도킹).
- **경력 팝업** — 히어로 카드의 "경력 한눈에 보기" 버튼이 경력 섹션을
  유리 패널 팝업(`<dialog>`)으로 띄웁니다. 본문 경력 섹션과 같은 데이터.
- **상단바** — 워드마크 · 섹션 링크(경력/프로젝트/기술/학력·활동) · 연락.
  좁은 화면에서는 가운데 링크가 가로 스크롤됩니다.
...
- 표정(mood): 기본 무표정 · `pix--smile` (입이 나타남)
- 동작(anim): 기본 아이들(숨쉬기+팔 흔들림+깜빡임) · `pix--wave` (오른팔 손 흔들기)
